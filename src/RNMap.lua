...
function RNMap:drawMapAt(x, y, tileset)
    ...
    if self.objectgroups[0]~=nil then
       self:createPhysicBodiesFromObjecLayer()            
    end   
    
end

function RNMap:drawMapLayer(x, y, tileset)        
        local layerdraw = self.layers[0]
        layerdraw:drawLayerParallax(x, y, tileset)
        self.layerdraw = layerdraw
        print(layerdraw.renderedMap.prop)    
    --return layerdraw
end
