...
function RNMapLayer:drawLayerParallax(x, y, tileset)
    if self.renderedMap ~= nil then
        self.renderedMap:remove()
    end
    local image = MOAIImage.new()
    image:init(config.width + tileset:getTileWidth(), config.height + tileset:getTileHeight())
    local newMap = image
    --collectgarbage("step")
    ...
    self.renderedMap = RNFactory.createImageFromMoaiImageWithoutAdd(newMap)
