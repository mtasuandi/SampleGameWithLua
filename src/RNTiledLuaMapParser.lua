...
function load(map, filename)
    ...
    if mapLua.properties then
        map.properties = {}
        local size = loadProperties(map.properties, mapLua.properties)
        ...
        for key, value in pairs(mapLua.layers) do
            ...
            if value.type == "tilelayer" then
                ...
                if value.properties then
                    ...
                    local propertiesSize = loadProperties(map.layers[map.layersSize].properties, value.properties)
            ...
            elseif value.type == "objectgroup" then
                ...
                if value.properties then
                    ...
                    local size = loadProperties(map.objectgroups[map.objectgroupsSize].properties, value.properties)
                ...
                if value.objects then
                    for key, value in pairs(value.objects) do
                        ...
                        if value.properties then
                            ...
                            local size = loadProperties(map.objectgroups[map.objectgroupsSize].objects[currentName].properties, value.properties)
...
function loadProperties(targetMapItem, value)
